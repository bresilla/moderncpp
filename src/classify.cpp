...
int main(int argc, char *argv[]) {
  if (argc != 4)
    throw std::runtime_error("Usage: ./exe modelName modelLabels imageName");
...
torch::Tensor read_image(const std::string &imageName) {
  ...
  torch::Tensor img_tensor =
      torch::from_blob(img.data, {img.rows, img.cols, 3}, c10::kFloat);
  img_tensor = img_tensor.permute({2, 0, 1});
  img_tensor.unsqueeze_(0);

  img_tensor =
      torch::data::transforms::Normalize<>(norm_mean, norm_std)(img_tensor);
